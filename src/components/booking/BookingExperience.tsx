...
import { bondCalendarDateKey, calendarDateKeyNow } from "@/lib/bond-calendar-date";
...
import { bookingOptionalAddons, estimateSelectedAddonsTotal } from "@/lib/product-package-addons";
...
function formatBookingDateLong(isoDate: string): string {
  const d = new Date(`${isoDate}T12:00:00`);
  if (Number.isNaN(d.getTime())) return isoDate;
...
export function BookingExperience() {
  ...
  const themeStyle = useMemo(
    () => resolveBookingThemeStyle(portal ?? undefined, env.ok ? env.devTheme : undefined, appearanceMode),
    [portal, env, appearanceMode]
  );
  ...
  const { state, pushBookingState } = useBookingUrlState(portal, searchParams);
  ...
  const visitPassParsed = useMemo(
    () => (selectedProductForHooks ? parseVisitPassProduct(selectedProductForHooks) : null),
  ...
  const punchRemaining = visitPassParsed
    ? remainingVisitsForProduct(punchWallet, visitPassParsed.productId)
    : 0;
  ...
  useEffect(() => {
    ...
    if (remainingVisitsForProduct(punchWallet, visitPassParsed.productId) >= seed) {
  ...
  }, [env, visitPassParsed, punchWallet, searchParams]);
  ...
  const scheduleSlotsContext = useMemo(() => {
    ...
    const durationMinutes =
      visitPassParsed != null ? visitPassParsed.durationMinutes : state?.duration ?? undefined;
    return { ...scheduleSettingsContext, duration: durationMinutes };
  }, [scheduleSettingsContext, visitPassParsed, state?.duration]);
  ...
  const prefetchProductSchedule = useCallback(
    (productId: number) => {
      ...
      const parsed = product ? parseVisitPassProduct(product) : null;
  ...
  const toggleSlot = useCallback(
    (resourceId: number, resourceName: string, s: ScheduleTimeSlotDto) => {
      ...
      setSelectedSlots((prev) => {
        ...
        const catalogListUnit = isVisitPassProduct(productRow) ? null : cashUnitPriceForBondFallback(productRow);
        const rawUnit =
  ...
  const visitPassCheckout = useMemo(() => {
    if (visitPassParsed == null) return null;
    return resolveVisitPassCheckout(visitPassParsed, punchRemaining, pickedSlotsOrdered.length);
  }, [visitPassParsed, punchRemaining, pickedSlotsOrdered.length]);

  const selectedPunchWalletEntry = visitPassParsed
  ...
  const productsForRail = useMemo(() => {
    ...
    return [...rows].sort((a, b) => {
      const aOwned = isVisitPassProduct(a) && remainingVisitsForProduct(punchWallet, a.id) > 0;
      const bOwned = isVisitPassProduct(b) && remainingVisitsForProduct(punchWallet, b.id) > 0;
      if (aOwned === bOwned) return 0;
      return aOwned ? -1 : 1;
    });
  }, [productsQuery.data?.data, punchWallet]);
  ...
  const detailPunchRemaining = useMemo(() => {
    ...
    if (p == null || !isVisitPassProduct(p)) return null;
  ...
  const punchPassExtrasTotal = useMemo(() => {
    if (visitPassParsed == null || selectedAddonIds.size === 0) return 0;
    return estimateSelectedAddonsTotal({
      addons: packageAddons,
      selectedIds: selectedAddonIds,
      slots: pickedSlotsOrdered,
      targeting: addonSlotTargeting,
      quantities: addonQuantities,
      slotQuantities: addonSlotQuantities,
    });
  }, [
    visitPassParsed,
    selectedAddonIds,
  ...
  if (portalQuery.isPending) {
    ...
    return (
      <main
        ...
        style={hydrated ? resolveBookingThemeStyle(undefined, env.devTheme, appearanceMode) : undefined}
        aria-busy="true"
        aria-live="polite"
      >
  ...
  const facilityTimeZone =
    portal.options.facilities.find((f) => f.id === state.facilityId)?.timezone ??
    scheduleQuery.data?.resources.find((row) => row.timeSlots.some((s) => s.timezone.length > 0))?.timeSlots.find(
      (s) => s.timezone.length > 0
    )?.timezone;
  const todayDateKey = calendarDateKeyNow(facilityTimeZone);
  const scheduleSlotDateKeys = new Set<string>();
  for (const row of scheduleQuery.data?.resources ?? []) {
    for (const slot of row.timeSlots) {
      const key = bondCalendarDateKey(slot.startDate);
      if (key != null) scheduleSlotDateKeys.add(key);
    }
  }
  const scheduleDisplayDate =
    scheduleSlotDateKeys.size === 1 ? [...scheduleSlotDateKeys][0]! : state.date;
  const slotPriceCurrency = selectedProduct?.prices?.[0]?.currency ?? null;
  const punchPassExtrasLabel =
    punchPassExtrasTotal > 0
      ? formatPrice(punchPassExtrasTotal, slotPriceCurrency ?? visitPassParsed?.packPrice?.currency ?? "USD")
      : null;
  ...
  const scheduleDurationMinutes =
    visitPassParsed != null
      ? visitPassParsed.durationMinutes
  ...
  const setProduct = (productId: number) => {
    ...
    const parsed = p ? parseVisitPassProduct(p) : null;
    pushBookingState({
      ...state,
      productId,
    ...
    >
      ...
      <div className="mt-6 flex flex-col gap-12">
        ...
        <section aria-labelledby="products-heading" className="text-left">
          ...
          <div className={soloProduct ? "cb-services-rail cb-services-rail--solo cb-hide-scrollbar" : "cb-services-rail cb-hide-scrollbar mt-4"}>
            {productsForRail.map((p) => {
              ...
              const punchMeta = parseVisitPassProduct(p);
              const punchOwnedEntry = punchMeta
                ? walletEntryForProduct(punchWallet, punchMeta.productId)
                : null;
              const ownedRemaining = punchOwnedEntry
                ? Math.max(0, punchOwnedEntry.remaining)
                : 0;
              ...
              const chipPrice = punchPackPrice
                ? { amount: punchPackPrice.amount, currency: punchPackPrice.currency }
                : catalogMinScaled
                  ? { amount: catalogMinScaled.min, currency: catalogMinScaled.currency }
                  : null;
              const cardTag = punchMeta != null
                ? ({ icon: <IconPassTicket className="size-3.5 shrink-0 opacity-95" />, label: tb("passTag") } as const)
                ...
                >
                  ...
                  >
                    <div className="cb-product-card-media">
                      ...
                      />
                      <div
                        className={`cb-product-chip cb-product-chip--price ${memberFreeChip ? "cb-product-chip--member-free" : ""} ${ownedRemaining > 0 ? "cb-product-chip--punch-remaining" : ""}`}
                      >
                        <span className="cb-product-chip-price-row">
                          {ownedRemaining > 0 && punchOwnedEntry != null ? (
                            ...
                            <>
                              <span className="cb-product-chip-price-amount">
                                {formatSlotCurrency(chipPrice.amount, chipPrice.currency)}
                              </span>
          ...
          {soloProduct ? (
            <aside className="cb-product-solo-info" aria-label={tb("productDetailAbout")}>
              <div className="cb-product-solo-info-head">
                ...
                {(() => {
                  const meta = parseVisitPassProduct(soloProduct);
        ...
        <section aria-labelledby="schedule-heading" className="text-left">
          ...
          {scheduleQuery.data && state.view === "matrix" && state.date ? (
            <div className="cb-matrix-date-nav-row mt-4">
              <span className="cb-matrix-date-nav-label">
                {formatBookingDateLong(scheduleDisplayDate ?? state.date)}
              </span>
              <div className="cb-matrix-date-nav-arrows">
                <button
                  type="button"
                  className="cb-matrix-date-nav-btn"
                  disabled={scheduleNavDates.prev == null}
                  onClick={() => scheduleNavDates.prev && setDate(scheduleNavDates.prev)}
                ...
                <button
                  type="button"
                  className="cb-matrix-date-nav-btn"
                  disabled={scheduleNavDates.next == null}
                  onClick={() => scheduleNavDates.next && setDate(scheduleNavDates.next)}
      ...
      {portal && state && (
        <>
          ...
          <ModalShell open={picker === "duration" && visitPassParsed == null} title={tb("selectDuration")} onClose={() => setPicker(null)}>
            <ListPickerBody
      ...
      <ProductDetailModal
        ...
        punchRemaining={detailPunchRemaining}
