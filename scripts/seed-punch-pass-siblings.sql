-- Clone hourly reservation products into punch-pass siblings (court pack, lesson pack, …).
--
-- A pack appears on a portal because the clone source already lives in a category
-- listed on that portal (Products."categoryId" ∈ Filters.options.categoriesIds).
-- Run scripts/discover-punch-pass-targets.sql first and paste clone_from ids below.
--
--   psql "$BOND_DATABASE_URL" -v ON_ERROR_STOP=1 -f scripts/seed-punch-pass-siblings.sql
--
-- Public API isPunchPass is Products."punchCard". Does not write PunchPasses.
-- Dry-run: change the final COMMIT to ROLLBACK.
-- Existing product names in the org are skipped.
...
BEGIN
	...
	VALUES
		(659887, 'Court 5-pack', 5, 60, 75),
		(0, 'Lesson 8-pack', 8, 60, 200);
	...
	LOOP
		...
		IF v_clone_product_id IS NULL OR v_clone_product_id <= 0 THEN
			INSERT INTO punch_pass_seed_result (
				...
				NULL, v_name, true, v_punches, v_duration_minutes,
				NULL, NULL, v_clone_product_id,
				0, 0, 0, 'set clone_from from discover-punch-pass-targets.sql'
			);
			CONTINUE;
		END IF;
